...
(function ($, window, document, undefined) {
    ...
    function validate(variable, defaultValue) {
        if (variable && $.isNumeric(variable) && variable > 0) return variable;
        return defaultValue;
    }
    ...
    function createSequences(rows, cols, method) {
        if ($.isArray(method)) return method;
    ...
    var sequenceMethods = {
        lr: function (rows, cols) {
            ...
            for (var j = 0; j < cols; ++j) {
                var step = [];
                for (var i = 0; i < rows; ++i) step.push([i, j]);
                sequences.push(step);
            }
            return sequences;
        },
        ...
        tb: function (rows, cols) {
            ...
            for (var i = 0; i < rows; ++i) {
                var step = [];
                for (var j = 0; j < cols; ++j) step.push([i, j]);
                sequences.push(step);
            }
            return sequences;
        },
        ...
        lrtb: function (rows, cols) {
            var sequences = [];
            for (var i = 0; i < rows; ++i)
                for (var j = 0; j < cols; ++j) sequences.push([[i, j]]);
            return sequences;
        },
        ...
        rltb: function (rows, cols) {
            var sequences = [];
            for (var i = 0; i < rows; ++i)
                for (var j = cols - 1; j >= 0; --j) sequences.push([[i, j]]);
            return sequences;
        },
        ...
        tblr: function (rows, cols) {
            var sequences = [];
            for (var j = 0; j < cols; ++j)
                for (var i = 0; i < rows; ++i) sequences.push([[i, j]]);
            return sequences;
        },
    ...
    var animation = {
        ...
        prepare: function (options) {
            ...
            if (options.variables.adjustDuration) subOptions.duration /= 2;

            if (!$.isArray(effects)) effects = [effects];
            var effectsOptions = [];
            for (var i = 0; i < effects.length; ++i) {
                var effect = effects[i];
                if (typeof effect != 'object') effect = { effect: effect };
            ...
            for (var i = 0; i < sequences.length; ++i) {
                ...
                if (options.variables.sequent) {
                    var count = i;
                    if (parseInt(count / cycle) % 2 == 0) count = count % cycle;
                    else count = cycle - (count % cycle);
                ...
                for (var j = 0; j < step.length; ++j) {
                    ...
                    var effectOptions =
                        effectsOptions[n % effectsOptions.length];
